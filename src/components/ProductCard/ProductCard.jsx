...
const ProductCard = ({ product }) => {
  const { title, price, oldPrice, rating = 0, reviews = 0, freeShipping } = product;
  const { addToCart } = useCart();
  const displayImage = product.img || product.image || 'https://via.placeholder.com/300x300?text=n11';

  return (
    <div className="product-card">
      <div className="product-image-container">
        <img src={displayImage} alt={title} className="product-image" />
